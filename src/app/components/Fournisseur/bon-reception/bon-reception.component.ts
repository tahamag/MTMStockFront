...
@Component({
  ...
  styleUrl: './bon-reception.component.css'
})
  ...
  filterTransactions(): void {
    ...
    this.filteredTransactions.set(
      this.transactions().filter(transaction => {
        ...
        const matchesIntervenant = isNaN(intervenantId) || intervenantId === -1 || transaction.fournisseur?.id === intervenantId;
// afterStart && beforeEnd &&
  ...
  loadTransactions(): void {
    ...
    this.transactionService.getTransactions('fournisseurs').subscribe({
      next: (response) => {
        if (response) {
  ...
  onEdit(transaction: Transaction): void {
    ...
    this.transactionForm.patchValue({
      ...
      Id_intervenant: transaction.fournisseur?.id,
