...
export class FactureComponent {
  ...
  filterTransactions(): void {
    ...
    this.filteredFactures.set(
      this.factures().filter(facture => {
        ...
        const matchesIntervenant = isNaN(intervenantId) || intervenantId === -1 || facture.client?.id === intervenantId;
  ...
  loadFacturations(): void {
    ...
    this.factureService.getFactures("clients").subscribe({
      next: (response) => {
        if (response) {
  ...
  onEdit(facture: Facture): void {
    ...
    this.factureForm.patchValue({
      ...
      Id_intervenant: facture.client?.id,
